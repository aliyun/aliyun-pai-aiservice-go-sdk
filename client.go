...
import (
	...
	"github.com/aliyun/aliyun-pai-aiservice-go-sdk/util"
)
...
// APIClient manages communication with the AI model Service Restful API API v1.0.0
...
func NewClient(host, appId, token string) *APIClient {
	...
	cfg := NewConfiguration(host, appId, token)
	...
	// API Services
	c.AiCreationApi = (*AiServiceAiCreationApiService)(&c.common)
	c.ImageApi = (*AiServiceImageApiService)(&c.common)
	c.TextApi = (*AiServiceTextApiService)(&c.common)
	c.VideoApi = (*AiServiceVideoApiService)(&c.common)
...
func (c *APIClient) prepareRequest(
	...
	if c.cfg.HostAddress != "" {
		localVarRequest.Host = c.cfg.HostAddress
	}
...
func (c *APIClient) createSign(cfg *Configuration, path string, method string,
	postBody interface{},
	queryParams url.Values) string {
	...
	signStr := util.BuildSignString(method, path, buffer, queryParams)

	return util.CreateSign(signStr, cfg.token)
}
